...
def find_peak(x_psd_welch, y_psd_welch_magnitude_dB, border_min, border_max, distance):
    print('HEY SHOW ME YOURE ALIVE')
    ## NEED TO ENSURE BORDERS ARE WELL-DEFINED FOR EFFECTIVE PEAK SEARCHING
    psd_welch_peaks_location, _ = find_peaks(y_psd_welch_magnitude_dB, height=(border_min, border_max), distance=distance)
...
def psd_plot_peak_finding(sc: signal_collection, bin_width, emitter, receiver):
    ...
    border_min = -97
    border_max = -50
    # Should only output 2 peaks:
    distance = 1000
    #x_psd_welch_peaks, y_psd_welch_peaks, local_peaks = find_peak(x_psd_welch, y_psd_welch_magnitude_dB, border_min, border_max)
    x_psd_welch_peaks, y_psd_welch_peaks, local_peaks = find_peak(x_psd_welch, y_psd_welch_magnitude_dB, border_min, border_max, distance)
    np.append(matrix_peaks, local_peaks, axis=0)
    #print(f'MATRIX PEAKS: {matrix_peaks}')
    # Averaging
    y_avg_1 = np.mean(matrix_peaks[:, 2])
    y_avg_2 = np.mean(matrix_peaks[:, 3])
    return y_avg_1, y_avg_2
...
for excitation_frequency in allowed_frequencies:
    # Choose 1 excitation frequency:
    for cycles in allowed_cycles:
        # We will measure coherence of signals by comparing all received signals across cycles relative to baseline '0' and excluding 'AI' and 'Healthy' states.
        for emitter in allowed_emitters:
            # Path determination 1:
            for receiver in allowed_receivers:
                # Path determination 2:
                f, Pxy = csd(x_freq_baseline: allowed_cycles='0', x_freq_all_but_baseline(cycles and not '0'))
                f, Cxy = coherence(x_freq_baseline: allowed_cycles='0', x_freq_all_but_baseline(cycles and not '0'))
                coherence_index = np.mean(Cxy)
'''