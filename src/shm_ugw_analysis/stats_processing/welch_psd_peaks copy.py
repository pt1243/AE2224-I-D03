...
def find_psd_peaks(
    ...
    y_psd_welch_magnitude_dB: np.ndarray = gaussian_filter1d(y_psd_welch_magnitude_dB, sigma=2)
...
def plot_signal_collection_psd_peaks(sc: Iterable[Signal], bin_width: int | float, file_label: str = None) -> None:
    ...
    cmap1 = mpl.color_sequences['Blues']
    for s in sc:
        ...
        ax.plot(psd[0], 10*np.log10(psd[1]), color=cmap,
...
def plot_signal_psd_peaks(
    ...
    y_psd_welch_magnitude: np.ndarray = np.abs(y_psd_welch)
    y_psd_welch_magnitude_dB: np.ndarray = 10*np.log10(y_psd_welch_magnitude)
    # y_psd_welch_magnitude_dB = y_psd_welch_magnitude
    ...
    cmap = mpl.colormaps['Blues']
    # fig, ax = plt.subplots(1, 1, figsize=(12, 10))
    ax.plot(x_psd_welch, y_psd_welch_magnitude_dB, color=cmap)
